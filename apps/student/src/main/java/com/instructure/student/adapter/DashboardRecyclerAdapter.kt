...
class DashboardRecyclerAdapter(
    ...
    override fun loadData() {
        ...
        mApiCalls = tryWeave {
            ...
            val contexts = favoriteCourses + favoriteGroups
            val conferences = ConferenceManager.getLiveConferencesAsync(contexts, isRefresh).await().dataOrNull
                ?.filter {
                    // Remove blacklisted (i.e. 'dismissed') conferences
                    !blackList.contains(it.id.toString())
                }
                ?.onEach {
                    // Attempt to add full canvas context to conference items, fall back to generic built context
                    val genericContext = CanvasContext.fromContextCode("${it.contextType}_${it.contextId}")!!
                    it.canvasContext = when (it.contextType) {
                        "course" -> mCourseMap[it.contextId] ?: genericContext
                        "group" -> groupMap[it.contextId] ?: genericContext
                        else -> genericContext
                    }
                } ?: emptyList()
